...
void NumberedPipeCommand::createPipe(Pipe& currPipe) { 
  currPipe = CmdHandler::getNumberPipeList().get(m_number); // get already wroten pipe, and prepare to write again
  if (currPipe.empty()) {
    currPipe.create();
    CmdHandler::getNumberPipeList().set(m_number, currPipe);
  }
}
